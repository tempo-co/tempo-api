...
describe('SessionService', () => {
	...
	describe('initializeSessionMetadata', () => {
		...
		it('should initialize metadata if session exists, has user, and no existing metadata', async () => {
			...
			expect((mockRequest.session as MockSession)?.metadata?.userAgent).toBe(mockRequest.headers?.['user-agent']);
			...
			expect((mockRequest.session as MockSession)?.metadata?.clientDescription).toBe('Chrome on Windows 10');
